# ============================================================
# Binary-Relevance Panel for Multi-Label HMD Detection (Patient-Level Evaluation)
#
# Purpose
#   - Read per-subject .xlsx files containing windowed annotations and landmark-distance trajectories
#   - Extract time-series features per window
#   - Train one binary classifier per symptom (binary relevance) with patient-level CV (no leakage)
#   - Aggregate window probabilities to patient-level probabilities (configurable aggregation)
#   - Tune per-label decision thresholds on TRAIN patients only (with optional control constraints)
#   - Report patient-level metrics, per-label metrics, controls-only FP/FPR diagnostics
#   - Optionally run nested-CV per-label best-model selection (train-only), then evaluate on test
#
# Dependencies
...
from scipy.special import expit  # for sigmoid(decision_function) fallback
...
# Configuration
# -------------------------
data_folder = "../datasets/dataset_lc/"   # <- change to dataset_dd/ if needed
output_dir = "./"
...
# Threshold grid for per-label threshold search
THRESH_GRID = np.linspace(0.01, 0.99, 199)

# ---- Label-specific feature-selection overrides (optional)
...
# ---- Label-specific clinical-cost weighting (alpha = weight on FNR)
...
# ---- Label-specific override application toggles
APPLY_V71B_TO_SVM = True
APPLY_V71B_TO_OTHERS = True

# ---- Nested CV per-label best-model selection (optional; does not remove other outputs)
...
# Suggested starting point (patient-level)
RECALL_MIN_BY_LABEL = {
    "Dystonia": 0.70,
    "Tremor": 0.70,
    "Myoclonus": 0.50,
...
# 0) FP_controls + FPR_controls + spec_min + recall_min
# 1) drop spec_min
# 2) drop FPR_controls
# 3) drop recall_min
# 4) drop all constraints (pure min error)
...
# -------------------------
# Nested: constrained threshold selection for single label
# -------------------------
...
def select_threshold_min_error_under_constraints(
    ...
    under constraints (depending on relax_level).
    Constraints are TRAIN-only (inner-train in nested CV).
    ...
    def _constraints_ok(y_true, y_pred):
        ...
        # controls constraints (evaluated on controls subset only)
        if has_controls:
            ...
            if relax_level <= 3:
                # FP_controls is the strongest "safety" constraint: keep it until relax_level 4
    ...
    # If infeasible, fall back to unconstrained min error at t=0.5 (caller will handle relaxation)
...
# -------------------------
# Safe calibration helpers (LinearSVC)
# -------------------------
def safe_calibrate_or_raw_linear_svc(base_linear_svc, y_train_bin, method="sigmoid", max_cv=3):
    ...
    # If minority class has <2 samples, any K-fold calibration fails.
    if m < 2:
...
def decision_function_to_proba(estimator_or_pipeline, X_eval):
    ...
    This is a fallback when predict_proba is not available or calibration is not feasible.
    """
...
def build_pipeline_generic(scaler_or_none, estimator, y_train_bin, label=None, n_features=None, enable_smote=False, apply_v71b=False):
    ...
    Unified builder:
      ...
      - supports label-specific K if apply_v71b=True
    """
    steps = []
...
# -------------------------
# Model specs (for non-SVM models)
# -------------------------
...
# -------------------------
# Run meta (saved in output workbook)
# -------------------------
run_meta = {
...
all_rows = []
per_label_rows = []
patient_store = {}  # store_key -> {"y_true":[...], "y_pred":[...]}
    ...
    for scaler_name, scaler in scalers.items():
        for svm_hp in SVM_GRID:
            ...
            for fold, (p_tr, p_te) in enumerate(mskf.split(unique_subjects, subject_Y), start=1):
                ...
                # Aggregate windows -> patient probs
                ...
                # Patient-level GT (OR across windows)
                ...
                # Controls mask on TRAIN patients
                ...
                # Per-label metrics (patient-level)
                for j, lab in enumerate(symptoms):
                    yt = subj_y_test[:, j]
                    yp = subj_pred_test[:, j]
...
if ENABLE_OTHER_MODELS:
    ...
    for spec in specs:
        ...
        # single-scaler per spec (does NOT affect SVM branch)
        ...
        for scaler_name, scaler_obj in scaler_iter:
            ...
            for fold, (p_tr, p_te) in enumerate(mskf.split(unique_subjects, subject_Y), start=1):
                ...
                for j, lab in enumerate(symptoms):
                    ...
                    # Safe calibration for LinearSVC when needed
...
# -------------------------
# Nested CV: choose best model per label on TRAIN only; evaluate on TEST
# -------------------------
nested_rows = []
nested_label_rows = []
nested_patient_store = {}  # for decomposition keyed by "NestedLabelSelection"
...
def enumerate_all_candidate_configs():
    ...
    Candidate list for nested label selection:
      ...
      - all other-model specs (their single scaler / or Identity)
    """
    candidates = []
...
def fit_predict_label_proba(candidate, X_tr, y_tr_bin, X_eval, label, apply_v71b, enable_smote):
    ...
    # Safe calibration for LinearSVC candidates when needed
    ...
    # Fallback: hard predictions as pseudo-proba
    try:
...
def inner_score_candidate_constrained_error(
    ...
    # Need BOTH inner-tr proba (to tune threshold) and inner-val proba (to evaluate)
    ...
    # Aggregate to patient-level (single label)
    ...
    # Tune threshold on TRAIN patients only with relaxation if needed
    chosen_t = None
    chosen_diag = None
    ...
    # If still infeasible: fall back to 0.5 (should be extremely rare with relax_level=4)
    if chosen_t is None:
        chosen_t = 0.5
...
if ENABLE_NESTED_LABEL_MODEL_SELECTION:
    ...
    nested_patient_store[nested_store_key + "__choices"] = []  # store dict per fold
    nested_patient_store[nested_store_key + "__choices_meta"] = []  # diagnostics per fold
    ...
    for fold, (p_tr, p_te) in enumerate(mskf.split(unique_subjects, subject_Y), start=1):
        ...
        # Inner CV setup (auto-reduce if needed)
        ...
        # Build patient-level inner labels for TRAIN subjects only
        ...
        for j, lab in enumerate(symptoms):
            # If label absent/constant on TRAIN subjects, select None and handle as constant predictor
            ...
            # Evaluate each candidate with inner CV using constrained error minimization
            for cand in candidates:
                ...
                if mean_err < best_err:
                    best_err = mean_err
                    best_candidate = cand
                    # keep a representative meta summary
                    best_meta = {
        ...
        # Train chosen per-label models on FULL outer-train and predict train+test window probs
        ...
        # Final threshold tuning on OUTER-TRAIN
        thresholds = tune_thresholds_policy(
            subj_y_train,
            subj_proba_train,
            labels=symptoms,
...
# -------------------------
# Summaries + Error decompositions + Save
# -------------------------
results_df = pd.DataFrame(all_rows)
per_label_df = pd.DataFrame(per_label_rows)

# Controls FPR/FP by fold
controls_rows = []
...
# ---- Best-by criteria and error decompositions
