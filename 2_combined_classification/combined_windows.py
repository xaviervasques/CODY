# === MULTI-LABEL SYMPTOM CLASSIFICATION SCRIPT ===
import os
import numpy as np
import pandas as pd
import warnings
    ...
    for clf_name, grid in param_grids.items():
        for params in ParameterGrid(grid):
            ...
            if clf_name == 'XGBoost':
              ...
              base_clf = LogisticRegression(**params, random_state=random_state)

            elif clf_name == 'SVM':
              base_clf = SVC(**params, probability=True, random_state=random_state)
            ...
            fold_metrics = {'F1_1': [], 'F1_0': [], 'Accuracy': [], 'ROC-AUC': []}

            for fold, (train_idx, test_idx) in enumerate(skf.split(X, y.argmax(axis=1), groups)):
                ...
                try:
                  ...
                  try:
                    y_proba = np.vstack([p[:, 1] for p in clf.predict_proba(X_test)]).T
                  ...
                  print(f"  ❌ Error in fold {fold+1} for {clf_name} with scaler {scaler_name}: {e}")
                  continue
            ...
            all_results.append({
                ...
                **params,
                ...
                'ROC_AUC_mean': np.nanmean(fold_metrics['ROC-AUC']),
                'ROC_AUC_std': np.nanstd(fold_metrics['ROC-AUC'])
            })
            pd.DataFrame(all_results).to_excel(os.path.join(output_dir, f"temp_multilabel_windows_results_LC.xlsx"), index=False)
    
if not all_results:
    print("❌ No results to save. Likely an issue with data extraction or classifier execution.")
else:
    
    pd.DataFrame(all_results).to_excel(os.path.join(output_dir, f"multilabel_windows_comparison_LC.xlsx"), index=False)
